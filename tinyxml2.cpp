...
//自定义函数，根据结点名称获取节点指针
XMLElement* queryNodeByName(XMLElement* parentNode, const string& name)
{
    XMLElement* node = parentNode->FirstChildElement();
    while (node != NULL)
    {
        if (node->Name() == name)
            break;
...
//异常处理机制
int exceptionHandler(const char* xmlPath, const char* exp, string& message)
{
    message += exp;
    if (strcmp("\n消息:输入参数不全\n", exp) == 0)
    {
        outputInitialToXML(xmlPath, message);
        return 1;
    }
    else
    {
        outputErrorToXML(xmlPath, message);
        return -1;
    }
}
//自定义函数,初始化状态
int outputInitialToXML(const char* xmlPath,string & message)
{
    XMLDocument doc;
    ...
    nodeIO = queryNodeByName(root, "status"); nodeIO->SetText("");
    //将所有输出量值空
    nodeIO = queryNodeByName(root, "out");
    nodeClassify = nodeIO->FirstChildElement();
    while (nodeClassify != NULL)
    {
        node= nodeClassify->FirstChildElement();
        while (node != NULL)
        {
            node->SetText("");
            node = node->NextSiblingElement();//下一个兄弟节点
        }
        nodeClassify = nodeClassify->NextSiblingElement();
    }
    return doc.SaveFile(xmlPath);
}

//自定义函数,输出Error
int outputErrorToXML(const char* xmlPath, string& message)
{
    XMLDocument doc;
    int res = doc.LoadFile(xmlPath);
    if (res != 0)
    ...
    XMLElement* node, * nodeIO, * nodeClassify;
    nodeIO = queryNodeByName(root, "message"); nodeIO->SetText(message.c_str());
    nodeIO = queryNodeByName(root, "status"); nodeIO->SetText("Error");

    return doc.SaveFile(xmlPath);
}
//将字符串转换为浮点数
void stringToData(double& data, const char* string)
{
    if (string == nullptr)
    {
        throw("\n消息:输入参数不全\n");
...
//将字符串转换为整数
void stringToData(int& data, const char* string)
{
    if (string == nullptr)
    {
        throw("\n消息:输入参数不全\n");
...
//将字符串给字符串赋值
void stringToData(char* string1, const char* string2)
{
    if (string2 == nullptr)
    {
        throw("\n消息:输入参数不全\n");
...
//将字符串给字符串赋值
void stringToData(string & string1, const char* string2)
{
    if (string2 == nullptr)
    {
        throw("\n消息:输入参数不全\n");
